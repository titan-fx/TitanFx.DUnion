...
namespace TitanFx.DUnion.GeneratorTests;
...
public partial class SourceGeneratorTests
{
    ...
    private const string _testCasesPrefix = "TitanFx.DUnion.GeneratorTests.TestCases.";
    ...
    private static void AssertNoDiff(string actual, string expected)
    {
        ...
        Assert.Fail(
            "Expected the generated source(s) to match the expectation, but there were differences:\n"
                + diffStr
        );
    }
    ...
    {
        using var stream = _assembly.GetManifestResourceStream(
            $"{_testCasesPrefix}{name}{_expectedPostfix}"
        );
        if (stream is null)
            return null;
    ...
    {
        using var stream = _assembly.GetManifestResourceStream(
            $"{_testCasesPrefix}{name}{_testCasesPostfix}"
        )!;
        using var sr = new StreamReader(stream);
        return sr.ReadToEnd();
    }
    ...
    private string RunSourceGenerator(string source)
    {
        ...
        var tree = CSharpSyntaxTree.ParseText(
            source,
            options: new(preprocessorSymbols: preprocessorSymbols)
        );
        ...
        var generated = result.GeneratedTrees.Where(x =>
            !x.FilePath.StartsWith(
                "TitanFx.DUnion\\TitanFx.DUnion.SourceGenerator\\TitanFx.DUnion."
            )
        );
