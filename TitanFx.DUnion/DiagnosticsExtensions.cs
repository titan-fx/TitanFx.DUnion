...
internal static class DiagnosticsExtensions
{
    ...
    private static readonly CA.DiagnosticDescriptor _unionConstructorMustCallAnotherConstructor =
        new(
            id: $"{_id}006",
    ...
    public static void AttributeArgumentMustBeAnIdentifier(
        this GeneratorContext ctx,
        CA.AttributeData attribute,
        int index
    )
    {
        ctx.AddDiagnostic(
            _attributeArgumentMustBeAnIdentifier,
            [LocationOfCtorArgument(attribute, index)],
            [
    ...
    public static void AttributeOnlyOnCase(
        this GeneratorContext ctx,
    ...
    {
        ctx.AddDiagnostic(
            _attributeOnlyOnCase,
            attributes.GetLocations(ctx.CancellationToken),
            [attributeName]
        );
    }

    public static void AttributeOnlyOnCaseTypeParameter(
        this GeneratorContext ctx,
        string attributeName,
        IEnumerable<CA.AttributeData> attributes
    )
    {
        ctx.AddDiagnostic(
            _attributeOnlyOnCaseTypeParameter,
            attributes.GetLocations(ctx.CancellationToken),
            [attributeName]
        );
    }

    public static void AttributeOnlyOnClassOrStruct(
        this GeneratorContext ctx,
        string attributeName,
        IEnumerable<CA.AttributeData> attributes
    )
    {
        ctx.AddDiagnostic(
            _attributeOnlyOnClassOrStruct,
            attributes.GetLocations(ctx.CancellationToken),
            [attributeName]
        );
    }

    public static void AttributePropertyMustBeAnIdentifier(
        this GeneratorContext ctx,
        CA.AttributeData attribute,
        string propertyName
    )
    {
        ctx.AddDiagnostic(
            _attributePropertyMustBeAnIdentifier,
            [LocationOfPropertyArgument(attribute, propertyName)],
            [propertyName, attribute.AttributeClass?.ToFullSignature() ?? "UNKNOWN"]
        );
    }

    public static void AttributePropertyMustBeSpecificType(
        this GeneratorContext ctx,
        CA.AttributeData attribute,
        string propertyName,
        Type type
    ...
    {
        ctx.AddDiagnostic(
            _attributePropertyMustBeSpecificType,
            [LocationOfPropertyArgument(attribute, propertyName)],
            [
                propertyName,
                attribute.AttributeClass?.ToFullSignature() ?? "UNKNOWN",
    ...
    {
        var refKeyword = @case
            .DeclaringSyntaxReferences.Select(r => r.GetSyntax(ctx.CancellationToken))
        ...
        ctx.AddDiagnostic(
            _caseCannotBeARefStruct,
            refKeyword.Length == 0 ? @case.GetLocations() : refKeyword,
            []
        );
    }

    public static void CaseCannotBeLessAccessibleThanTheUnion(
        this GeneratorContext ctx,
        CA.INamedTypeSymbol union,
        CA.INamedTypeSymbol @case
    )
    {
        ctx.AddDiagnostic(
            _caseCannotBeLessAccessibleThanTheUnion,
            @case.GetLocations(),
            [
    ...
    public static void CaseIsConfiguredButAlsoExcluded(
        this GeneratorContext ctx,
        IEnumerable<CA.AttributeData> exclude,
        IEnumerable<CA.AttributeData> config
    )
    {
        ctx.AddDiagnostic(
            _caseIsConfiguredButAlsoExcluded,
            exclude
                .GetLocations(ctx.CancellationToken)
                .Concat(config.GetLocations(ctx.CancellationToken)),
            []
        );
    }

    public static void DuplicateAttribute(
        this GeneratorContext ctx,
        string attributeName,
        IEnumerable<CA.AttributeData> attributes
    )
    ...
    public static void IncompatibleTypeParameters(
        this GeneratorContext ctx,
        TypeId case1,
        TypeParameter case1Parameter,
        TypeId case2,
        TypeParameter case2Parameter
    )
    {
        ctx.AddDiagnostic(
            _incompatibleTypeParameters,
            case1Parameter.Location,
            [
                case1Parameter.Name,
    ...
    public static void UnionCannotHaveABaseType(
        this GeneratorContext ctx,
        CA.INamedTypeSymbol unionType
    )
    {
        ctx.AddDiagnostic(_unionCannotHaveABaseType, unionType.GetLocations(), []);
    }

    public static void UnionCannotHaveInstanceAutoProperties(
        this GeneratorContext ctx,
        CA.IPropertySymbol property
    )
    {
        ctx.AddDiagnostic(_unionCannotHaveInstanceAutoProperties, property.GetLocations(), []);
    }

    public static void UnionCannotHaveInstanceFields(
        this GeneratorContext ctx,
        CA.IFieldSymbol field
    )
    {
        ctx.AddDiagnostic(_unionCannotHaveInstanceFields, field.GetLocations(), []);
    }

    public static void UnionConstructorMustCallAnotherConstructor(
        this GeneratorContext ctx,
        CA.IMethodSymbol ctor
    )
    {
        ctx.AddDiagnostic(_unionConstructorMustCallAnotherConstructor, ctor.GetLocations(), []);
    }

    public static void UnionsShouldHave2OrMoreCases(
        this GeneratorContext ctx,
        IEnumerable<CA.AttributeData> attributes
    )
    {
        ctx.AddDiagnostic(
            _unionsShouldHave2OrMoreCases,
            attributes.GetLocations(ctx.CancellationToken),
            []
        );
    }
    ...
    private static IEnumerable<Location?> GetLocations(
        this IEnumerable<CA.AttributeData> attributes,
    ...
    private static Location? LocationOfCtorArgument(CA.AttributeData? attribute, int index)
    {
        ...
        return (attrSyntax as CA.CSharp.Syntax.AttributeSyntax)
                ?.ArgumentList?.Arguments.Where(a => a.NameEquals is null)
    ...
    private static Location? LocationOfPropertyArgument(CA.AttributeData? attribute, string name)
    {
        if (attribute?.ApplicationSyntaxReference?.GetSyntax() is not { } attrSyntax)
            return null;
        return (attrSyntax as CA.CSharp.Syntax.AttributeSyntax)
                ?.ArgumentList?.Arguments.FirstOrDefault(a => a.NameEquals?.Name.ToString() == name)
                ?.GetLocation()
            ?? attrSyntax.GetLocation();
    }
}
