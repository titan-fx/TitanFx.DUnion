...
using TitanFx.DUnion.Models;
using static Microsoft.CodeAnalysis.CSharpExtensions;
using static TitanFx.DUnion.Models.Constants;
...
{
    public static Union? Parse(
        CA.ISymbol symbol,
        CA.SemanticModel model,
        CA.AttributeData[] attributes,
        GeneratorContext context
    )
    {
        if (
            symbol
            is not CA.INamedTypeSymbol
            {
                TypeKind: CA.TypeKind.Class or CA.TypeKind.Struct
            } caseHolder
        )
        {
        ...
        {
            TypeParameters = new(
                id.TypeParameters.Select(p => (id, p))
                    .Concat(
                        cases
                            .Select(c => c.Case)
                            .SelectMany(c => c.Id.TypeParameters, (c, p) => (c.Id, p))
                    )
                    .GroupBy(p => p.p.Name)
                    .Select(g =>
                    {
                        ...
                        foreach (var parameter in parameters.Skip(1))
                        {
                            if (constraints.SymmetricExcept(parameter.p.Constraints).Count > 0)
                                context.IncompatibleTypeParameters(
                                    parameter.Item1,
                                    parameter.p,
                                    first.Item1,
                                    first.p
    ...
    private static Func<CA.ITypeParameterSymbol, string> CreateTypeParameterNameRemapping(
        CA.INamedTypeSymbol symbol,
        GeneratorContext context
    )
    {
        var typeParameterNameRemapping = symbol
            .TypeParameters.Select(
                KeyValuePair<CA.ISymbol, string> (p) =>
                {
                    var attrs = p.GetAttributes()
                        .Where(attr =>
                            attr.AttributeClass?.ToDisplayString() == DUnionGenericAttribute
                        )
                        .ToArray();
                    if (attrs.Length == 0)
                        return new(p, p.Name);
                    ...
                    if (
                        attr is not { ConstructorArguments: [{ Value: string name }, ..] }
    ...
    private static string? GetAttributeIdentifierProperty(
        IReadOnlyDictionary<string, AttributeProperty> properties,
        string propertyName,
        GeneratorContext context,
        out CA.AttributeData? attribute
    )
    {
        if (
            GetAttributeProperty<string>(properties, propertyName, context, out attribute)
            is not string name
        )
            return null;
    ...
    private static T? GetAttributeProperty<T>(
        IReadOnlyDictionary<string, AttributeProperty> properties,
        string propertyName,
        GeneratorContext context,
        out CA.AttributeData? attribute
    ...
    private static IReadOnlyDictionary<string, AttributeProperty> GetProperties(
        IEnumerable<CA.AttributeData> attributes
    )
    {
        return attributes
            .SelectMany(
                a => a.NamedArguments,
                (attr, arg) => (arg.Key, new AttributeProperty(arg.Value, attr))
            )
            .GroupBy(a => a.Key)
    ...
    {
        return symbol
            .DeclaringSyntaxReferences.Select(r => r.GetSyntax(token))
            .OfType<PropertyDeclarationSyntax>()
            .SelectMany(s => s.AccessorList?.Accessors ?? [])
            .Any(a => a is { Body: null, ExpressionBody: null });
    }

    private static bool IsCaseExcluded(
        ILookup<string?, CA.AttributeData> attributeNameLookup,
        GeneratorContext context
    )
    {
    ...
    private static UnionCaseWithSymbol? ParseCase(
        CA.INamedTypeSymbol symbol,
        GeneratorContext context
    )
    {
        ...
        var attributeNameLookup = symbol
            .GetAttributes()
            .ToLookup(attr => attr.AttributeClass?.ToDisplayString());
    ...
    private static UnionCaseConfig ParseCaseConfig(
        string name,
        CA.AttributeData[] attributes,
        GeneratorContext context
    )
    {
        ...
        if (
            GetAttributeIdentifierProperty(GetProperties(attributes), IsCaseName, context, out _)
            is string isCaseName
        )
            config = config with { IsCaseName = isCaseName };
        if (
            GetAttributeIdentifierProperty(
                GetProperties(attributes),
                CaseOrDefaultName,
                context,
                out _
            )
            is string caseOrDefaultName
        )
            config = config with { CaseOrDefaultName = caseOrDefaultName };
        return config;
    }

    private static TypeId ParseTypeId(
        CA.INamedTypeSymbol symbol,
        Func<CA.ITypeParameterSymbol, string> getSymbolName
    )
    {
        ...
        var typeParameters = symbol.TypeParameters.Select(p =>
            ParseTypeParameter(p, getSymbolName)
        );

        return new TypeId(ns, new(containers), name, new(typeParameters), GetLocations(symbol));
    }

    private static TypeParameter ParseTypeParameter(
        CA.ITypeParameterSymbol symbol,
        Func<CA.ITypeParameterSymbol, string> getSymbolName
    )
    {
        ...
        if (symbol.HasReferenceTypeConstraint)
            constraints.Add(
                symbol.ReferenceTypeConstraintNullableAnnotation == CA.NullableAnnotation.Annotated
                    ? "class?"
                    : "class"
            );
    ...
    private static UnionConfig ParseUnionConfig(
        CA.AttributeData[] attributes,
        GeneratorContext context
    )
    {
        ...
        return new UnionConfig(
            GetAttributeIdentifierProperty(properties, DiscriminatorName, context, out _)
                ?? result.DiscriminatorName,
            GetAttributeIdentifierProperty(properties, ValueName, context, out _)
                ?? result.ValueName,
            GetAttributeIdentifierProperty(properties, SwitchName, context, out _)
                ?? result.SwitchName,
            GetAttributeIdentifierProperty(properties, MatchName, context, out _)
                ?? result.MatchName,
    ...
    )
    {
        if (model.Compilation.GetTypeByMetadataName(Helpers.ToFullName(id)) is { } existingUnion)
        {
            ...
            return new(
                existingUnion,
                new(
                    existingUnion.DeclaredAccessibility,
                    existingUnion.TypeKind,
                    existingUnion.IsRecord
                )
            );
        }
        ...
        return new(
            caseHolder,
            new(caseHolder.DeclaredAccessibility, caseHolder.TypeKind, caseHolder.IsRecord)
        );
    }
    ...
    private static void ReportIllegalBaseType(CA.INamedTypeSymbol union, GeneratorContext context)
    {
        if (
            union.BaseType
            is not { SpecialType: CA.SpecialType.System_Object or CA.SpecialType.System_ValueType }
        )
            context.UnionCannotHaveABaseType(union);
    }

    private static void ReportIllegalConstructors(
        CA.INamedTypeSymbol type,
        GeneratorContext context
    )
    {
        foreach (
            var ctor in type
                .Constructors.Where(ctor => !ctor.IsStatic)
                .Where(ctor => ctor.DeclaringSyntaxReferences.Length > 0)
                .Where(ctor =>
                    !ctor
                        .DeclaringSyntaxReferences.Select(r =>
                            r.GetSyntax(context.CancellationToken)
                        )
    ...
    private static void ReportInstanceFields(GeneratorContext context, CA.INamedTypeSymbol union)
    {
        ...
        foreach (
            var property in members[CA.SymbolKind.Property]
                .OfType<CA.IPropertySymbol>()
                .Where(x => IsAutoProperty(x, context.CancellationToken))
        )
            context.UnionCannotHaveInstanceAutoProperties(property);
    }

    private static void ReportStaticUnions(
        CA.INamedTypeSymbol definition,
        GeneratorContext context,
        CA.INamedTypeSymbol union
    )
    {
        if (union.IsStatic)
    ...
    private static void ValidateCaseAccessibility(
        TypeDefinitionWithSymbol union,
        UnionCaseWithSymbol[] cases,
        GeneratorContext context
    )
    {
    ...
    private readonly record struct TypeDefinitionWithSymbol(
        CA.INamedTypeSymbol Symbol,
