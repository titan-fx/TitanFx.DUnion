...
internal static class GeneratorOutputs
{
    ...
    public static void GenericNonUnionCaseTypeParameters(
        CA.IncrementalGeneratorInitializationContext context
    )
    {
        Report(
            context,
            context.SyntaxProvider.ForAttributeWithMetadataName(
                ...
                static (ctx, token) =>
                {
                    ...
                    if (!IsValid(ctx.TargetSymbol))
                        report.AttributeOnlyOnCaseTypeParameter(
                            DUnionGenericAttribute,
                            ctx.Attributes
                        );
        ...
        context.RegisterPostInitializationOutput(static context =>
        {
            foreach (var name in _assembly.GetManifestResourceNames())
            {
                if (name.EndsWith(".cs"))
                {
                    ...
                    context.AddSource(
                        name,
                        Helpers.ToSourceText(_public.Replace(source, "internal", 1))
                    );
                }
            }
        });
    ...
    private readonly record struct UnionState(
        Union? Model,
        GeneratorContext Context,
        Models.Location Location
    );

    private static UnionState ParseUnionAttribute(
        CA.GeneratorAttributeSyntaxContext context,
    ...
    private static void Report(
        CA.IncrementalGeneratorInitializationContext context,
        CA.IncrementalValuesProvider<GeneratorContext> reporter
    )
    {
        context.RegisterSourceOutput(
            reporter,
            static (ctx, reporter) => reporter.SendDiagnostics(ctx)
        );
    }
}
