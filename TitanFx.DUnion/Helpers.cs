...
internal static class Helpers
{
    ...
    public static string GetNamespace(this CA.ISymbol symbol)
    {
        ...
        var @namespace = string.Join(
            ".",
            namespaceStack.Select(n => n.Name).Where(n => n.Length > 0).Reverse()
        );
        return @namespace;
    }
    ...
    public static bool IsUnionCase(CA.ISymbol symbol)
    {
        return symbol is CA.INamedTypeSymbol
            ...
            && symbol
                ...
                .Any(attr => attr.AttributeClass?.ToDisplayString() == Constants.DUnionAttribute);
    }

    public static string Render(TypeId id)
    {
        var result = id
            .Containers.Select(c => c.Name + Render(c.TypeParameters))
